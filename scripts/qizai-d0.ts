...
const SHOTS: { id: string; file: string; ref: string[]; prompt: string }[] = [
  ...
  },
  {
    ...
    ref: ["front", "sit", "side"],
    prompt:
      "近景平视：七仔蹲坐着，嘴里叼着一枚圆形扁平的金属小瓶盖（像啤酒瓶盖，绝不是贝壳），" +
      "抬眼看向画面外偏下方——视线明显偏离镜头轴线，" +
      "神态平静、专注、不讨好。环境：码头木箱一角，黄昏，大量留白。" +
      `${IDENTITY}${STYLE_SCENE}。${FORBID}、任何亲昵讨好的神态。`,
  },
  {
  ...
  },
  {
    ...
    ref: ["side", "walk"],
    prompt:
      "中景：黄昏的小路上，七仔停在一间小木屋前的侧影，头转向木屋门口——门口台阶上放着那枚小瓶盖，" +
      "门上挂着一块空白的木门牌。此镜头它嘴里没有叼东西（瓶盖已经放在门口）。" +
      "环境：安静的小屋、空白门牌、暖黄的天色。" +
