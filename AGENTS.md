...
6. **图片一律走导演系统（2026-08-04 拍板，doc/15）**：日常画面**禁止按事件调用生图**——用 `lib/visual/` 导演+合成（`direct()` → `composeMoment()`），素材来自资产库（场景×时段 + `CatPose` 姿势集）。允许调用生成的只有三类：**新猫入岛资产**（领养链路自动：立绘→相遇照片→姿势集，`lib/visual/poses.ts`；漏了由 tick 每日安全网补，手动补产才用 `scripts/poses.ts`）、**新场景资产**（`scripts/scenes.ts` + `scripts/scene-times.ts` 四时段，且要同步登记 `lib/visual/assets.ts` 的场景映射/比例/锚点三张表）、**L5 特殊时刻**（相遇/D7/重大剧情/节日，低频高价值）。比例观感用 `composeMoment` 的 `scaleOverride` 三档对比校准，改表不改逻辑。
